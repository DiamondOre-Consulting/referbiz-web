    ...
    const fetchAssociates = async () => {
      try {
        ...
        const response = await axios.get(
          "http://https://referbiz-web.onrender.com:8080/api/admin-rb/admin-employees-data",
          {
            headers: {
      ...
      <div className="flex flex-col justify-center items-center py-10 px-10 bg-gray-200">
        ...
        <div className="grid grid-cols-1 py-10 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {associates?.map((associate) => (
            <Link
              to={`/admin-all-employees/${associate._id}`}
              key={associate._id}
              ...
              <div>
                {associate?.profileImage ? (
                  <img
                    ...
                    src={`http://https://referbiz-web.onrender.com:8080/` + associate?.profileImage}
                    alt="avatar"
                  />
                ) : (
                  <img
                ...
                <div>
                  <h3 className="text-lg font-semibold">{associate.EmpName}</h3>
                  <p className="text-sm text-gray-600">{associate.EmpEmail}</p>
                </div>
              </div>
