...
const AffTotalOffered = () => {
    ...
    const [afftototaloff, setAffTotalOff] = useState([]);
    ...
    useEffect(() => {
        ...
        const fetchAllOffered = async () => {
            try {
                ...
                const response = await axios.get('https://referbiz-web-backend.onrender.com/api/candidates/my-offered-refs', {
                    headers: {
                            ...
                            afftototaloff.map((off) => {
                                return (
                                    <tr class="bg-gray-100 border-b dark:bg-gray-800 dark:border-gray-700 text-center">
                                        ...
                                        <td class="px-6 py-4">
                                            {off.refUniqueEmailId}
                                        </td>
                                        <td class="px-6 py-4">
                                            {off.refPhone}
                                        </td>
                                        <td class="px-6 py-4">
                                            <a href={off.PDF} target="_blank" className='text-blue-600 underline'> veiw </a>
                                        </td>
                                    </tr>
                                )
                            })
...
export default AffTotalOffered